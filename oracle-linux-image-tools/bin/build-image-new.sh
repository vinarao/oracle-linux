...
load_env() {
  ...
  if [[ -z "${VM_NAME}" ]]; then
    VM_NAME="Cloud-Bridge-${CLOUD}-${BUILD_NUMBER}"
  fi
...
#  cp /etc/oracle/rpms/cloudbridge_appliance.rpm "${WORKSPACE}/${PACKER_FILES}"
}
...
run_packer() {
  ...
  cd "${WORKSPACE}"
  echo_header "entering ${WORKSPACE}"
  ...
  echo_message "${PACKER} build ${PACKER_BUILD_OPTIONS} ${VM_NAME}.json"
